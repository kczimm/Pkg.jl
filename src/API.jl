...
"""
    gc(ctx::Context=Context(); collect_delay::Period=Day(7),
                               spaces_cleanup_period::Period=Day(31),
                               kwargs...)
...
accessed for at least `spaces_cleanup_period` days, defaulting to twenty-one days.  The
...
function gc(ctx::Context=Context(); collect_delay::Period=Day(7),
                                    spaces_cleanup_period::Period=Day(21),
                                    kwargs...)
    Context!(ctx; kwargs...)
    env = ctx.env
    ...
    # Write condensed Scratchspace usage
    ...
    function process_scratchspace(path, packages_to_delete)
        ...
        # `spaces_cleanup_period`, then do not mark this path
        if now() - last_access > spaces_cleanup_period
            return nothing
        end
        return [path]
    end
    ...
    for depot in depots()
            ...
            for uuid in readdir(scratchdir)
                ...
                for space in readdir(uuid_dir)
                    ...
                    if !(space_dir in spaces_to_keep)
                        push!(depot_orphaned_spaces, space_dir)
                    end
                end
            end
        end
        ...
        merge_orphanages!(new_orphanage, depot_orphaned_spaces, spaces_to_delete, old_orphanage)
